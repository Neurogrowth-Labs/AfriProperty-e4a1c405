...
import { BanknotesIcon, ShieldCheckIcon, AcademicCapIcon, MoonIcon, UserGroupIcon, BuildingStorefrontIcon, SparklesIcon, MapIcon, VrHeadsetIcon, ShareIcon, LinkIcon, CheckIcon, HeartIcon, HeartIconSolid, ClockIcon, TicketIcon, BikeIcon, CalendarIcon, CpuChipIcon } from './icons/ActionIcons';
...
type Tab = 'overview' | 'community' | 'utilities' | 'ai-guide';
                    ...
                    <main className="w-full md:w-2/3 lg:w-3/4 overflow-y-auto">
                        <div className="border-b border-slate-200 dark:border-slate-700 px-6">
                            <nav className="-mb-px flex space-x-6 overflow-x-auto">
                                ...
                                <TabButton id="utilities" label="Transport" activeTab={activeTab} setActiveTab={setActiveTab} />
                                <TabButton id="ai-guide" label="AI Local Guide" activeTab={activeTab} setActiveTab={setActiveTab} />
                            </nav>
                        </div>
                         <div className="p-6">
                            ...
                            {activeTab === 'ai-guide' && <AIGuideTab neighborhood={selectedNeighborhood} />}
                        </div>
                    </main>
                </div>
            </div>
...
({ id, label, activeTab, setActiveTab }) => (
    <button 
        ...
        className={`py-3 px-1 border-b-2 font-semibold text-sm whitespace-nowrap ${activeTab === id ? 'border-brand-primary text-brand-primary' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
    >
        {label}
    </button>
);
...
const OverviewTab: React.FC<{ neighborhood: NeighborhoodGuide }> = ({ neighborhood }) => (
    <div className="space-y-6">
        <div className="grid grid-cols-5 gap-6">
            ...
            <div className="col-span-5 lg:col-span-2 aspect-[4/3] rounded-lg overflow-hidden border dark:border-slate-700">
...
// --- AI Guide Tab Component (Using Google Maps Grounding) ---
const AIGuideTab: React.FC<{ neighborhood: NeighborhoodGuide }> = ({ neighborhood }) => {
    ...
    const handleAsk = async () => {
        ...
        try {
            ...
            // Use gemini-2.5-flash with googleMaps tool as per guidelines
            const response = await ai.models.generateContent({
                model: 'gemini-2.5-flash',
                contents: `Answer the following question about ${neighborhood.name} (${neighborhood.coordinates.lat}, ${neighborhood.coordinates.lng}): ${query}`,
                config: {
                    tools: [{ googleMaps: {} }],
                    toolConfig: {
                        retrievalConfig: {
    ...
    return (
        <div className="space-y-6">
            <div className="bg-brand-light dark:bg-slate-800 p-6 rounded-lg">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2 flex items-center gap-2">
                    <CpuChipIcon className="w-6 h-6 text-brand-primary" />
                    Ask a Local Expert (AI)
                </h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">
                    Powered by Google Maps data. Ask about restaurants, parks, schools, or anything nearby.
                </p>
                <div className="flex gap-2">
                    <input 
                        value={query} 
                        onChange={(e) => setQuery(e.target.value)} 
                        placeholder="e.g. What are the top rated coffee shops nearby?" 
                        className="flex-grow px-4 py-3 rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700"
                        onKeyPress={(e) => e.key === 'Enter' && handleAsk()}
                    />
                    <button 
                        onClick={handleAsk} 
                        disabled={isLoading}
                        className="bg-brand-primary text-white font-semibold px-6 py-3 rounded-lg hover:bg-opacity-90 disabled:opacity-50"
                    >
                                     ...
                                     if (source.maps?.uri) {
                                         return (
                                            <li key={index} className="text-xs">
                                                <a href={source.maps.uri} target="_blank" rel="noopener noreferrer" className="text-brand-primary hover:underline">
                                                    {source.maps.title || 'Google Maps Location'}
                                                </a>
                                            </li>
                                         )
                                     }
